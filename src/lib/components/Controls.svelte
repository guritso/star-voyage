...
<div class="flex items-center gap-2">
  <div class="flex items-center gap-2">
    ...
    <button aria-label="Reset" title="Reset" class="w-10 h-10 flex items-center justify-center bg-gray-800 rounded" on:click={resetSim}>
    ...
    <button aria-label="Decelerate" title="Decelerate" class="w-10 h-10 flex items-center justify-center bg-gray-800 rounded" on:click={() => { speedVal = Math.max(0.01, speedVal / 2); }}>
    ...
    <button aria-label="Step Back" title="Step Back 1y" class="w-10 h-10 flex items-center justify-center bg-gray-800 rounded" on:click={stepBackward}>
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" class="text-white">
        <polygon points="16,6 8,12 16,18" />
        <rect x="18" y="6" width="2" height="12" />
      </svg>
    </button>
    <button aria-label="Play/Pause" title="Play/Pause" class="w-10 h-10 flex items-center justify-center bg-gray-800 rounded" on:click={togglePlay}>
      {#if playing}
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" class="text-white">
    ...
    <button aria-label="Step Forward" title="Step Forward 1y" class="w-10 h-10 flex items-center justify-center bg-gray-800 rounded" on:click={stepForward}>
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" class="text-white">
        <polygon points="8,6 16,12 8,18" />
        <rect x="4" y="6" width="2" height="12" />
      </svg>
    </button>
    <button aria-label="Accelerate" title="Accelerate" class="w-10 h-10 flex items-center justify-center bg-gray-800 rounded" on:click={() => { speedVal = Math.min(1000, speedVal * 2); }}>
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" class="text-white">
